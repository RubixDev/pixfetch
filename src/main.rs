...
fn main() {
    ...
    ].iter().filter(|(_, v)| v != &None).map(|(k, v)| (*k, v.to_owned().unwrap())).collect();
    ...
    let img_filename = match info.iter().find(|e| e.0 == "OS") {
        Some(distro) => {
            ...
            if distro == "Arch Linux" { col = 4; "logos/arch.png" }
            else if distro.contains("Android") { col = 2; "logos/android.png" }
            else if distro.contains("Debian") { col = 1; "logos/debian.png" }
            else { col = 3; "logos/tux.png" }
        },
        None => { col = 3; "logos/tux.png" },
    };
    ...
    for line in 0..(img.len().max(info.len())) {
        ...
        if line < info.len() {
            if info[line].0 == "u@h" {
                ...
                print!("\x1b[1;3{}m{: <9}\x1b[0m {}", col, info[line].0.to_owned() + ":", info[line].1);
            }
        }
        println!();
    }
